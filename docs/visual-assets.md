...
2026-09-05 사용자가 지정한 15개 업체 목록과 공식 홈페이지 주소를 기준으로, 각 회사의 공식 홈페이지에 공개된 로고를 회사 식별 및 해당 홈페이지 연결 목적으로 사용한다. 한국수력원자력 로고는 사용자가 직접 제공한 PNG로 교체했으며, 잘못 식별됐던 동대문구 이미지는 저장소와 배포 산출물에서 제거했다.
...
- OES·대원종합이엔지·유림기술·비케이비전의 제공 주소는 HTTP만 사용한다. 로고 이미지는 사이트 내부에 저장해 혼합 콘텐츠를 방지하지만 외부 이동 후 연결 보안은 해당 업체 홈페이지 운영 범위다.
