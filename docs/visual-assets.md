...
2026-09-05 사용자가 지정한 16개 업체 목록과 공식 홈페이지 주소를 기준으로, 각 회사의 공식 홈페이지에 공개된 로고를 회사 식별 및 해당 홈페이지 연결 목적으로 사용한다. 한국수력원자력 로고는 사용자가 직접 제공한 PNG로 교체했으며, 잘못 식별됐던 동대문구 이미지는 저장소와 배포 산출물에서 제거했다.

- 공개 파일: `public/images/partners/`
- 회사명·표시순서·링크·파일 연결: `src/data/organizations.ts`
- 로고는 형태·색상을 재디자인하지 않고 비율을 유지한다. 흰색 문자가 포함된 두 로고는 원본 식별성을 위해 네이비 배경 위에 표시한다.
- 모든 링크는 새 창으로 열리며 `noopener noreferrer`를 적용한다.
- OES·대원종합이엔지·유림기술·비케이비전·리얼게인의 제공 주소는 HTTP만 사용한다. 로고 이미지는 사이트 내부에 저장해 혼합 콘텐츠를 방지하지만 외부 이동 후 연결 보안은 해당 업체 홈페이지 운영 범위다.
